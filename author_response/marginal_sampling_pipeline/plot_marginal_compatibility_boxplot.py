"""
Step 2: Plot Marginal Sampling Compatibility Boxplot
====================================================
Replicate plot4RQ1.py style for Marginal Sampling data (K=100, no truncation)

Data: 6 datasets × Vanilla LR × 10 seeds = 60 data points per metric pair

Usage:
    python 02_plot_marginal_compatibility_boxplot.py

Output:
    figures/marginal/marginal_compatibility_boxplot.png
"""
...
from tools.config import DATASETS, list_group_fairness, list_individual_fairness, list_utilities, COLORS_PAIR
...
def read_marginal_compatibility_data():
    """Read all marginal sampling compatibility data from JSON files"""
    all_data = defaultdict(list)
    data_count = 0

    # Marginal sampling: 6 datasets × 1 classifier (lr) × 10 seeds
    for dataset in DATASETS:
        classifier = "lr"  # Only Vanilla LR
        for seed in range(10):
            compatibility_path = osp.join(
                script_dir, "../models", dataset, "vanilla",
                classifier, f"seed_{seed}", "interactions_marginal",
            ...
            if osp.exists(compatibility_path):
                ...
                for metric_pair, value in data.items():
                    ...
                    if len(parts) == 2:
                        ...
                        normalized_key = metric_pair
                    all_data[normalized_key].append(value)

    print(f"Successfully read {data_count} files")
    return all_data


def prepare_boxplot_data(all_data):
...
def create_boxplot(boxplot_data, metric_pairs, colors, categories):
    ...
    makedirs(output_dir)
    output_path = osp.join(output_dir, 'marginal_compatibility_boxplot.png')
...
def print_statistics(metric_pairs, categories):
    ...
    print(f"Total (dataset, classifier, seed) combinations per metric pair: {len(DATASETS) * 1 * 10}")
...
def main():
    print("=" * 60)
    print("Step 2: Plot Marginal Sampling Compatibility Boxplot")
    print("K=100, No Truncation")
    print("=" * 60)

    print("\nReading Marginal Sampling compatibility data...")
    all_data = read_marginal_compatibility_data()

    print(f"\nTotal metric pairs found: {len(all_data)}")

    boxplot_data, metric_pairs, colors, categories = prepare_boxplot_data(all_data)
    ...
    print("\nOr run: 03_plot_marginal_per_order_violin.py")
